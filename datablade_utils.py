...
# ---------------------------------------------------------------------------
# 1) I/O Helpers -- used during initialization and geometry setup
# ---------------------------------------------------------------------------

def extract_from_ises(file_path):
    """Extract inlet/outlet angles and Reynolds number from an ISES file.
    Used in the initialization stage."""
...
def extract_from_blade(file_path):
    """Read blade pitch from blade file. Used in initialization."""
    with open(file_path, "r") as f:
        next(f)
        tokens = f.readline().split()
...
def extract_from_outlet(file_path):
    """Read inlet Mach number and pressure ratio from outlet file."""
    with open(file_path, "r") as f:
        for _ in range(19):
            next(f)
        tokens = f.readline().split()
...
def read_selig_airfoil(path):
    """Return raw x,y arrays from a Selig formatted airfoil file."""
    x, y = [], []
...
def copy_blade_file(original_filename, blade_dir):
    """Copy a blade file to a databladeValidation variant."""
...
# ---------------------------------------------------------------------------
# 2) Geometry utilities -- used in mesh generation
# ---------------------------------------------------------------------------

def calculate_intersection_point(P0, vr, P3, vs, d):
    """Compute control points for a closed trailing edge (mesh stage)."""
    def promote(v):
        v = np.asarray(v, float).ravel()
        return np.array([v[0], v[1], 0.0]) if v.size == 2 else v
    P0 = promote(P0); vr = promote(vr)
    P3 = promote(P3); vs = promote(vs)
    v_norm = (P3 - P0); v_norm /= np.linalg.norm(v_norm)
    n = np.cross(v_norm, np.cross(v_norm, vr)); n /= np.linalg.norm(n)
    P2 = P0 + d * n; P2_P3 = P3 + d * n
...
def create_nurbs_curve(P0, CP1, CP2, P3, weights=None):
    """Return a degree-4 BSpline curve through the given points."""
    def promote_vec(v):
        arr = np.asarray(v, float).ravel()
        return np.array([arr[0], arr[1], 0.0]) if arr.size == 2 else arr
    P0_3 = promote_vec(P0); CP1_3 = promote_vec(CP1)
    CP2_3 = promote_vec(CP2); P3_3 = promote_vec(P3)
    PM = (CP1_3 + CP2_3) / 2.0
...
def sample_nurbs(curve, n_te):
    """Sample n_te points along the nurbs curve."""
...
def resample_side(x, y, n_pts):
    """Resample one side of the airfoil with arc-length parameterization."""
...
def process_airfoil_file(path, n_points=1000, n_te=60, d_factor=0.5):
    """Return closed and resampled geometry for mesh generation."""
    ...
    P0 = [x_ss0[-1], y_ss0[-1]]; P3 = [x_ps0[-1], y_ps0[-1]]
...
def compute_d_factor(wedge_angle_deg, axial_chord, te_thickness,
                     ref_angle=10.0, ref_offset=0.005,
                     clamp=(0.2, 1.0)):
    """Empirical factor for TE closure from wedge angle (mesh stage)."""
...
def fit_circle(x, y):
    """Least-squares circle fit for geometry post-processing."""
...
def compute_geometry(path_to_airfoil, pitch, n_points=1000, n_te=60,
                     d_factor_guess=0.5, frac_fit=0.02, n_circle=30):
    """Return geometric properties of the blade (mesh stage)."""
    ...
    v_to_SS = np.vstack([cssx(s_cmb)-cspx(s_cmb), cssy(s_cmb)-cspy(s_cmb)]).T
    ...
    idx_le = np.arange(n_circle); idx_te = np.arange(-n_circle,0)
...
# ---------------------------------------------------------------------------
# 3) Boundary layer and flow computations -- used in configuration and analysis
# ---------------------------------------------------------------------------

def boundary_layer_props(x, rhoFlow, velFlow, muFlow, ReTurb=5e5):
    """Flat-plate boundary layer correlations (mesh stage)."""
...
def compute_mux(Tx):
    ...
    return (1.458e-6 * Tx**1.5) / (Tx + 110.4)
...
def compute_Rex(rhox, Vx, axial_chord, mux):
    """Reynolds number based on axial chord."""
    return rhox * Vx * axial_chord / mux

def compute_Losses(P01, P06, P1, P6, gamma):
    """Return different loss definitions for the cascade."""
    Loss_P0 = (P01 - P06) / P01
    Loss_P = (P01 - P06) / (P01 - P1)
    Loss_K = 1 - (1 - (P6/P06)**((gamma-1)/gamma)) / (1 - (P6/P01)**((gamma-1)/gamma))
    return Loss_P0, Loss_P, Loss_K

def compute_TurbulentQtys(TI, Umean, ILS, nu):
    """Turbulence quantities for SA or SST initialization."""
    C_mu = 0.09
    k = 3/2 * (Umean * TI)**2
...
def compute_TKE(TI, Umean):
    """Turbulent kinetic energy from intensity and velocity."""
    return 3/2 * (Umean * TI)**2

def compute_TKE_Dissipation(k, ILS):
    """TKE dissipation rate from integral length scale."""
    C_mu = 0.09
    return C_mu**(3/4) * k**(3/2)/ILS

def compute_Spec_Dissipation(k, ILS):
    """Specific dissipation (omega) from integral length scale."""
    C_mu = 0.09
    return C_mu**(3/4) * k**(1/2)/ILS
